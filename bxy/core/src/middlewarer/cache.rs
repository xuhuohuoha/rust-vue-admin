...
use crate::serve::utils::ApiUtils::ALL_APIS;
use crate::{
...
pub async fn add_cache_data(ori_uri: &str, api_key: &str, data_key: &str, data: String) {
    ...
    drop(res_data);
    tracing::info!(
        "add cache data,api_key: {}, data_key: {},api:{}",
        api_key,
        data_key,
        ori_uri
    );
...
pub async fn remove_cache_data(api_key: &str, api: Option<String>, data_key: Option<&str>) {
    let mut res_data = RES_DATA.lock().await;
...
pub async fn cache_fn_mid(req: Request, next: Next) -> Result<Response, StatusCode> {
        ...
        return match res_end.status() {
            StatusCode::OK => {
                tokio::spawn(async move {
                    remove_cache_data(&ctx.path.clone(), api_info.api.clone(), None).await;
                });
                Ok(res_end)
            }
            _ => Ok(res_end),
    ...
    let data_key = match api_info.data_cache_method.clone().as_str() {
        "1" => format!("{}_{}_{}", &ctx.ori_uri, &ctx.method, &token),
